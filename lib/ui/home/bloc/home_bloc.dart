...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  }) : super(HomeState.initial(homeRepository!, userRepository!,
            eventRepository!, assetRepository!, configRepository!));
  ...
  Stream<HomeState> mapEventToState(
    ...
    switch (event.runtimeType) {
      case OnApplyFilter:
        yield handleFilter(event as OnApplyFilter);
        break;
      case FetchAllData:
        ...
        yield* homeInteract!.handleAreaItemsResult(
            listResult[0] as Either<Failure, List<AreaItem>>, state);

        yield* homeInteract!.handleSuggestionResult(
            listResult[1] as Either<Failure, List<ActivityModel>>, state);

        yield* homeInteract!.handleCommunityResult(
            listResult[2] as Either<Failure, CommunityModel>, state);

        yield* homeInteract!.handleTrendingByWeekResult(
            listResult[3] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleTopRateResult(
            listResult[4] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleRestaurantsResult(
            listResult[5] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleFacilitiesResult(
            listResult[6] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleSportsResult(
            listResult[7] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleFoodTrucksResult(
            listResult[8] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleMightLikeResult(
            listResult[9] as Either<Failure, List<PlaceModel>>, state);

        yield* homeInteract!.handleEventsResult(
            listResult[10] as Either<Failure, List<EventInfo>>, state);
        break;
      case FetchWeatherInfo:
        ...
        break;
      case SelectFacility:
        if (state.currentArea!.filter!.isNotEmpty) {
