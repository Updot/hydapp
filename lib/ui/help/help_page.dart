...
class HelpPageState extends BaseState<HelpPage> {
  ...
  void initBasicInfo() {
    ...
    if (_helpBloc.state.email!.isNotEmpty) {
      _emailController.text = _helpBloc.state.email!;
    }
    if (_helpBloc.state.phone!.isNotEmpty) {
      ...
      showModalBottomSheet(
        ...
        builder: (BuildContext builder) {
          return CupertinoPickerView(
            ...
            listWidget:
                List<Widget>.generate(state.helpItems!.length, (int index) {
              return Center(
                child: MyTextView(
  ...
  void _onSenHelp(HelpState state) {
    ...
    if (!state.isDesValid! || _descriptionController.text.length < 10) {
      UIUtil.showToast(
        Lang.help_please_input_at_least_10_characters.tr(),
        backgroundColor: Colors.red,
      );
      return;
    }

    _pbLoading!.show(max: 100, msg: "Please wait ...");
