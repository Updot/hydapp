  ...
  Stream<SaveItemState> mapEventToState(SaveItemEvent event) async* {
    if (event is FetchSaveItem) {
      final defaultExperience =
          (state.listAreas!.length) > 0 ? state.listAreas![0].id : 0;
      final result =
      ...
      if (result != null && result.isNotEmpty) {
        state.listAreas!.forEach((element) {
          groupSaveItems[element.id!] =
              result.where((e) => (e.experience!.id) == element.id).toList();
        });
    ...
    yield result.fold(
      ...
      (value) {
        ...
        if (value != null && value.isNotEmpty) {
          state.listAreas!.forEach((element) {
            groupSaveItems[element.id!] =
                value.where((e) => (e.experience!.id) == element.id).toList();
          });
